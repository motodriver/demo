...
![投放自动化逻辑 (6)](/Users/lee/Downloads/投放自动化逻辑 (6).png)
...
- 广告账户、应用列表通过Bussiness Manager API拉取

包括调用这些广告账户、应用的Token也需要通过API生成
...
![模板示例图](./img/图片拼接/模板示例图.png)

- 插入GP图

运营选择图片类型、图片位置、图层顺序，文字内容。

![运营设计模板2](./img/图片拼接/运营设计模板2.jpg)

- 预览效果

将主题中的图填充进模板后，预览效果。此时可以选择保存模板。

![效果图2](./img/图片拼接/效果图2.jpg)
...
- 应用、Campaign、Adset、Ad四个层级的广告数据趋势查看

> 数据查看流程：**选择层级->选择目标->选择时间段->查看数据趋势->选择数据列**

> 注意：应用维度的数据无法直接拉取，需要拉取较低层级的数据后计算得到。
...
Campaign层级以下的数据&细分维度数据，调用接口即可



- 数据合并

应用维度的数据是不能直接调用接口拉取的，需要拉取Campaign的数据后重新计算的。所以需要额外的数据合并逻辑。
